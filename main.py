...
def train_rnn(net, data, val_data, loss, accuracy, batch, epochs, lr, train_lbs, val_lbs):
    ...
    for e in range(epochs):
        ...
        if (e % 10 == 0):
            val_loss, val_acc = validation(net, val_data, loss, accuracy, batch, val_lbs)
    ...
    return acc_list, error_list, valacc_list, valerror_list
...
def main():
	...
	acc_list, error_list, valacc_list, valerror_list = train_rnn(net, data=train_data, val_data=validate_data, loss=cross_entropy_loss, accuracy=accuracy, batch=100, epochs=100, lr=0.1, train_lbs=train_images['targets'], val_lbs=validate_images['targets'])
